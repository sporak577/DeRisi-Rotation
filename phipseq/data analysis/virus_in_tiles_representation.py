...
def plot_pie(series, label, outname):
    ...
    filtered = counts[counts / total >= 0.02] #filters counts to only keep labels that represent omore or equal 2% of total 
    other_sum = total - filtered.sum()
    
    if other_sum > 0: #sums how many items were excluded
        filtered["Other (<2%)"] = other_sum

    plt.figure(figsize=(8, 8))
    plt.pie(filtered, labels=filtered.index, autopct='%1.1f%%', startangle=140)
    plt.title(f"Tile Count Representation of Viral {label}")
    plt.tight_layout()
    plt.savefig(f"/Users/sophieporak/Desktop/{outname}_{date}.png")
    plt.close()
...
plot_pie(df['tax_rank_9'], "Strains", "virus_strain_distribution_pie")